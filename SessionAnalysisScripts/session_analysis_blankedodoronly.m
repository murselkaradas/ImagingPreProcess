...
addpath '/mnt/BigPurpleHPC/2Panalysis'
%% SVD  analysis for glomerular imaging
% SVD_2p_cluster_WS('/gpfs/scratch/karadm01/2Pdata/SC18/221012/2SEQ/aligned/SC18_221012_2SEQ_00001_00001.tif', 1, 40,100,60);
%%  FIELD 1
% Read H5 and roi file
path = '/gpfs/scratch/karadm01/2Pdata/M72/JG4983/220718/23odorsglom';
fieldname = 'JG4983_220718_23odorsglom';
img_format = [256, 256];
%img_format = [512, 512];
%fps =29.99;
fps = 58.18;
...
for tr=1:length(inh_onset)
    ...
    if ~isempty(inh_frame) && min(abs(double(frametrigger)-double(inh)))<200
...
%% Data format for JvG
%%
Session.UniqueConds =strings(size(OdorInfo.odors,1),1);
...
for i = 1: size(OdorInfo.odors,1)
    ...
    Session.UniqueConds(i) = convertCharsToStrings(OdorInfo.odors{i});
    Session.blockTrials{i} = ones(length(OdorInfo.odorTrials{i}),1);
end


save(strcat(fieldname,'JG.mat'), 'Session')
