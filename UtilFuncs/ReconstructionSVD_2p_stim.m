function [dff,trial_info,trials_read, meanAll,F]=ReconstructionSVD_2p_stim(U,SV,roiMasks,trial_info,pre,post)
...
for tr=1:num_trial
    ...
    if ~isempty(inh_frame)&&inh_frame+post_frame-1<=Nframe&&inh_frame-pre_frame>=1
        ...
        fcell=sv_upsample(round(single([inh-pre:inh+post-1])-frametrigger(inh_frame-pre_frame)),(s_lower:s_upper))*Ucell';
        baseline_frame=1:pre-60; %
