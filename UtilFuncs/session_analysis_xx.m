...
fieldname = 'MK18882_210702_rightbulbBENZ';
img_format = [256, 256];
fps = 58.18;
OdorDuration = 2;
...
for i = 1: size(OdorInfo.odors,1)
    ...
    plot([1 1]*0, ylim, '--b', 'LineWidth',1)
    plot([1 1]*OdorDuration*fps, ylim, '--b','LineWidth',1)
    plot(downsample((-pre:post-1),round(1000/fps))./round(1000/fps), mean(sniff_ds(:,OdorInfo.odorTrials{i}),2)-1, 'k', 'LineWidth',1)
    hold off
    title(OdorInfo.odors{i})
    xlabel('Frame number')
    ylabel('\DeltaF/F_0')
...
%%  LEFT SIDE
fieldname = 'MK18882_210702_leftbulbBENZ';
img_format = [256, 256];
fps = 58.18;
OdorDuration = 2;
