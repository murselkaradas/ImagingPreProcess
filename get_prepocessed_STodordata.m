...
for i = 1:wsNtrials
    if trials_read(i)
        try
            an = eval(sprintf('wsdata.sweep_%04d.analogScans', i));
            framediff = find(diff(an(:,1)) > 2);  % Frame trigger is channel 1
            FVdiff = find(diff(an(:, FVChannel)) > 2);
            ...
            if startPID < 0
                PIDmeas(kk,:) = vertcat(an(1, PIDChannel) * ones(abs(startPID) + 1, 1), an(1:min(end, endPID - 1), PIDChannel));
            else
                PIDmeas(kk,:) = an(max(1, startPID):min(end, endPID - 1), PIDChannel);
            end

            kk = kk + 1;
        catch ME
            warning('Error in trial %d: %s', i, ME.message);
