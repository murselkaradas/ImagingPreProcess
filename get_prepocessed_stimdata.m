function   get_prepocessed_stimdata(varargin)
    ...
    datamean = zeros(img_format); % Mean fluorescence image
    Fluo_cell = [];               % Fluorescence signal per ROI
    Nframestart = 0;              % Frame counter start
    datafull = [];                % Full data stack (if needed)
    
    % Determine TIFF file locations
    if usealigned_tiff
        tiff_names = dir(fullfile('aligned', strcat(fieldname, '_*.tif')));
        if isempty(tiff_names)
            disp('No TIFF files found in the aligned folder');
            % If no TIFFs in aligned folder, try the current folder
    ...
    % Extract filenames and folder paths
    tiff_filenames = {tiff_names.name};
    tiff_foldername = {tiff_names.folder};
    filenum = numel(tiff_names);
    
    % Error handling if no TIFF files found
    if filenum == 0
        error('Error: No TIFF files found.');
    else
    ...
    for i = 1:filenum
        fprintf('Processing file %d of %d: %s\n', i, filenum, tiff_filenames{i});
        
        % Load the TIFF file
        tiff_loaded = loadtiff(fullfile(tiff_foldername{i}, tiff_filenames{i}));
        
        % If difference image calculation is enabled, concatenate to full stack
        if calculate_diff_image
            datafull = cat(3, datafull, tiff_loaded);
        end
        
        % Convert TIFF data to double for processing
        data = double(tiff_loaded);
        Nframe = size(data, 3); % Number of frames in the TIFF
        
        % Update mean fluorescence image
        datamean = datamean + mean(data, 3);
        
        % Calculate fluorescence per ROI
        % cellMask_vec' is assumed to be a binary mask of ROIs in vector form
        reshaped_data = reshape(data, [img_format(1) * img_format(2), Nframe]);
        Fluo_cell = [Fluo_cell, double(cellMask_vec') * reshaped_data];
    end
    
    datamean = datamean / filenum; % Average across all TIFF files
    disp('TIF Processing complete.');
        ...
        if ~isempty(inh_frame)
            ...
            if max(sv_frame_range) < size(Fluo_cell,2) && (inh_frame>pre_inh) && inh_diff<1e3
                ...
                if calculate_diff_image
                    ...
                    img_df(:,:,:,ind) = int16(img_trial(:,:,pre_inh-round(fps/4):pre_inh+round(fps/2)-1) -img_baseline);
                end
                ind = ind+1;
            else
    ...
    if calculate_diff_image
        ...
        for i = 1: size(OdorInfo.odors,1)
            fprintf('odor %d is  getting processed \n',i)
